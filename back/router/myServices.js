...
Router.post("/AddMyService", async (req, res) => {
    const { Servicio, Id_user, Descripcion, Monto, Dia_pago } = req.body;
    ...
    try {
        ...
        const sql = `INSERT INTO ${tabla} (id_services, id_user, descripcion, monto, dia_pago) VALUES (?, ?, ?, ?, ?)`;
        await query(sql, [Servicio, Id_user, Descripcion, Monto, Dia_pago]);
...
Router.put("/updateMyServices", async (req, res) => {
    const { id_myservices, idServicio, id_user, descripcion, monto, diaPago, general_status } = req.body;
    ...
    try {
        const sql = `
            UPDATE ${tabla}
            SET id_services = ?, descripcion = ?, monto = ?, dia_pago = ?, general_status = ? 
            WHERE id_myservices = ? AND id_user = ?
        `;

        const result = await query(sql, [idServicio, descripcion, monto, diaPago, general_status, id_myservices, id_user]);
